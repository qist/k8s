...
################kubernetes:    wget https://storage.googleapis.com/kubernetes-release/release/v1.14.10/kubernetes-server-linux-amd64.tar.gz
...
###############kubectl下载  https://storage.googleapis.com/kubernetes-release/release/v1.14.10/kubernetes-client-linux-amd64.tar.gz 解压到/usr/bin
...
##########  etcd EVENTS 部署 ansible-playbook -i ${ETCD_EVENTS_IPS} events-etcd.yml
...
########## 如果cni配置没放到默认路径请创建软链 ln -s ${CNI_PATH}/etc /etc/cni 同时修改yaml hostPath路径 同时修改CALICO_IPV4POOL_CIDR 参数为 ${CLUSTER_CIDR}
